...
func handleViewMyAchievements(c telebot.Context, dbUser *models.User, appServices *services.AppServices) error {
	// Use the database primary key from the passed-in user object.
	userID := dbUser.ID // <-- MODIFIED
	log.Printf("[State Machine] UserID %d requesting their earned achievements.", userID)

	// GetUserProfile already fetches the user's earned achievements with all necessary details.
	userProfile, err := appServices.User().GetUserProfile(userID) // <-- USES CORRECT ID
	if err != nil {
		return SendServiceError(c, "fetching user profile for achievements", err)
	}

	if len(userProfile.Achievements) == 0 {
		return c.Send("شما هنوز هیچ دستاوردی کسب نکرده‌اید. به یادگیری ادامه دهید تا قفل آن‌ها باز شود!", keyboards.ProfileMenuKeyboard())
	}

	messageText := "این‌ها دستاوردهایی هستند که تا الان کسب کرده‌ای. برای مشاهده تصویر هرکدام، روی آن کلیک کن:"

	// Generate the inline keyboard with one button per earned achievement.
	achievementsKeyboard := keyboards.AchievementsListKeyboard(userProfile.Achievements)

	return c.Send(messageText, achievementsKeyboard)
}
...
func HandleStateBasedText(c telebot.Context, dbUser *models.User, appServices *services.AppServices) error {
	...
	case dbUser.LastMenu == StateProfileMenu:
		// User is in the profile menu.
		if userInput == keyboards.BtnViewAchievements.Text {
			return handleViewMyAchievements(c, dbUser, appServices)
		}
...
func handleCourseSelectionFromList(c telebot.Context, userInput string, userID uint, appServices *services.AppServices) error {
	...
	for _, courseSummary := range courses {
		...
		if userInput == courseSummary.PersianTitle {
			return displayCourseOverview(c, userID, courseSummary.ID, appServices, false)
		}
	}
		...
		if listErr != nil {
			...
			for _, courseSummary := range courses {
				...
				if userInput == btnText {
					return displayCourseOverview(c, dbUser.ID, courseSummary.ID, appServices, false)
				}
			}
		}
	}
...
func displayCourseOverview(
	...
	isQuizFinish bool, // this parameter is for when a quiz finish and right after we display courseoverview
) error {
	...
	if isQuizFinish {
		formattedText = formatters.EscapeMarkdownV2("برای ادامه یادگیری روی ادامه دوره بزنید.")
	}
